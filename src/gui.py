...
from sign_language_detection import baseMode
from sign_language_detection import collectionMode
...
class GUI(QMainWindow):
    ...
    def setupExampleSignSection(self):
        ...
        self.dropdownMenu.currentIndexChanged.connect(lambda: self.updateImage(self.dropdownMenu.currentText(), self.exampleImage))

        self.updateImage(self.dropdownMenu.currentText(), self.exampleImage)
    ...
    def setupAIOutputSection(self):
        ...
        self.updateImage('A', self.AIImage)
    ...
    def updateImage(self, letter, imageLabel):
    ...
    def noDetectionMode(self):
        ...
        self.activeThread = threading.Thread(target=baseMode, args=(self.camera, self.updateFrame, self.stopEvent), daemon=True)
    ...
    def dataCollectionMode(self):
        ...
        self.activeThread = threading.Thread(target=collectionMode, args=(self.camera, self.hands, self.mpDrawing, self.mpHands, self.updateFrame, self.getKeyPress, self.setCurrentKey, self.stopEvent), daemon=True)
        self.activeThread.start()
